...
def plot_hist(data_dict, task_strings=[], kdes=None, pop_m=None, pop_std=None, transform=None):
    ...
    if len(task_strings) < 1:
        ...
        fig, ax = plt.subplots(len(task_strings), num_metrics, figsize=(20, 4 * len(task_strings)), sharex='col')
        if len(ax.shape) < 2:
            # hack to to index the axes if there's only one task being plotted
            ax = np.array((ax,))
        for i, task_str in enumerate(task_strings):
            raw_data = deepcopy(data_dict[task_str])
            if pop_m is not None:
                raw_data = (raw_data - pop_m)/pop_std
            if transform is not None:
                raw_data = transform.transform(raw_data)
            num_metrics = raw_data.shape[1]
            # if kdes is not None:
            #     kde = kdes[task_str]
            ...
            for metric in range(num_metrics):
                ...
                ax_.plot(X, np.exp(kde.score_samples(X)))
                # if kdes is not None:
